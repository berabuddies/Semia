...
def _parse_statement(stmt: str, program: Program, anon_counter: list[int]) -> None:
    ...
    if arrow < 0:
        ...
        program.facts.setdefault(atom.relation, set()).add(
            tuple(arg.value for arg in atom.args)
        )
        return
...
def _expand_disjunctions(body_text: str, anon_counter: list[int]) -> list[tuple[Atom, ...]]:
    ...
    for chunk in chunks:
        ...
        if piece.startswith("(") and piece.endswith(")") and _is_disjunction(piece):
            inner = piece[1:-1]
            disjuncts = [
                _parse_body_atom(p.strip(), anon_counter)
...
def _parse_eq_constraint(
    ...
    if negated:
        raise ParseError(f"explicit negation on {op} not supported; use {'=' if op == '!=' else '!='}")
...
def _parse_atom(text: str, anon_counter: list[int]) -> Atom:
    ...
    args = tuple(
        _parse_term(part.strip(), anon_counter) for part in _split_top_level(inside, ",")
    )
