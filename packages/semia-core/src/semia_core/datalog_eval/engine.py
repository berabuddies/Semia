...
from .parser import Atom, ParseError, Program, Rule, Term, parse_dl_file
    ...
    for term in args:
        if term.is_var:
            ...
            if value is None:
                if allow_unbound:
                    raise EvalError(
                        f"head variable {term.value!r} unbound after body match"
                    )
                return None
            out.append(value)
        else:
    ...
    for head, deps in neg_edges.items():
        for q in deps:
            if pred_to_scc.get(head) == pred_to_scc.get(q):
                raise EvalError(
                    f"non-stratifiable negation in cycle involving {head!r} and {q!r}"
                )
...
def _toposort(nodes, deps: dict[int, set[int]]) -> list[int]:
    ...
    for start in sorted(nodes):
        ...
        work: list[tuple[int, Iterator[int]]] = [
            (start, iter(sorted(deps.get(start, ()))))
        ]
        on_stack.add(start)
        while work:
            node, it = work[-1]
