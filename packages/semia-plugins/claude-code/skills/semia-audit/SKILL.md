...
# Semia Audit for Claude Code

This Claude Code plugin uses the shared Semia audit workflow.
...
Claude Code performs synthesize in the current session. Deterministic Semia
...
`~/.claude/plugins/semia-audit`). The bundled binary is pure Python, has no
